...
def locations_snapshot():
    spark = None
    try: 
        logger = get_logger("bronze.erp_category_snapshot")
        logger.info("Starting ERP category snapshot job")
        spark = create_spark_session("bronze_category_snapshot")
        ...
        df = (
            ...
            .format("jdbc")
            .option("url", JDBC_URL)
            .option("dbtable", '"ERP".px_cat_g1v2')
        ...
        base_path = f"""
            s3a://bronze/snapshot/erp/category/load_date={load_date}
