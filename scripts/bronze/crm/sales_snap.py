...
def sales_detail_snapshot():
    spark = None
    try: 
        logger = get_logger("bronze.crm_sales_detail_snapshot")
        logger.info("Starting CRM sales_detail snapshot job")
        spark = create_spark_session("bronze_sales_detail_snapshot")
        ...
        df = (
            ...
            .format("jdbc")
            .option("url", JDBC_URL)
            .option("dbtable", '"CRM".sales_details')
        ...
        base_path = f"""
            s3a://bronze/snapshot/crm/sales/load_date={load_date}
